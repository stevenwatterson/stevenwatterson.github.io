...
**Idea 6 - CellWhere MyoNeuro: automated molecular network schema for neuromuscular disease**
...
Our existing CellWhere tool (https://www.sys-myo.com/cellwhere/) creates a schema of the cell and its compartments, building a MIN around a query list of proteins.  At present, CellWhere treats the cell as a generic structure.  We would like to transition CellWhere to become an open-source resource.  Coding will focus on web-design (PHP, Javascript, CSS, etc) and there is scope for great creativity in realizing beautiful, stylistic representations of the cell.  Along the way, we would extract cell schema for the neurons and muscle cells involved in neuromuscular disease. 
